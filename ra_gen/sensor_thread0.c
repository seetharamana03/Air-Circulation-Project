...
#if 1
static StaticTask_t sensor_thread0_memory;
#if defined(__ARMCC_VERSION)           /* AC6 compiler */
                static uint8_t sensor_thread0_stack[2048] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t sensor_thread0_stack[2048] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.sensor_thread0") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
TaskHandle_t sensor_thread0;
void sensor_thread0_create(void);
static void sensor_thread0_func(void *pvParameters);
...
{ .slave = 0x69, .addr_mode = I2C_MASTER_ADDR_MODE_7BIT, };
...
const rm_freertos_port_parameters_t sensor_thread0_parameters =
{ .p_context = (void*) NULL, };
...
#if 1
    sensor_thread0 = xTaskCreateStatic (
#else
                    BaseType_t sensor_thread0_create_err = xTaskCreate(
                    #endif
                                        sensor_thread0_func,
                                        (const char*) "Sensor Thread", 2048 / 4, // In words, not bytes
                                        (void*) &sensor_thread0_parameters, //pvParameters
                                        1,
#if 1
                                        (StackType_t*) &sensor_thread0_stack,
                                        (StaticTask_t*) &sensor_thread0_memory
#else
                        & sensor_thread0
                        #endif
                                        );
...
#if 1
    if (NULL == sensor_thread0)
    {
        rtos_startup_err_callback (sensor_thread0, 0);
    }
#else
                    if (pdPASS != sensor_thread0_create_err)
                    {
                        rtos_startup_err_callback(sensor_thread0, 0);
                    }
                    #endif
...
static void sensor_thread0_func(void *pvParameters)
{
    ...
    sensor_thread0_entry (pvParameters);
}
